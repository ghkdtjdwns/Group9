...
class Project {
    ...
    private func printProject(activities: [ActivitySoft], criticalActivities: [ActivitySoft]) {
        ...
        for activity in activities {
            ...
            if (activity.actualFinish != 0) {
                details.append("ID = \(activity.id): \(activity.description)\n" )
                details.append("Actual Start: \(activity.actualStart)\n")
                details.append("Actual Finish: \(activity.actualFinish)\n")
            } else if (activity.actualStart != 0) {
                details.append("ID = \(activity.id): \(activity.description)\n" )
                details.append("Actual Start: \(activity.actualStart)\n")
                details.append("Early Finish: \(activity.earlyFinish)\n")
                details.append("Late Finish: \(activity.lateFinish)\n")
                details.append("Total Float: \(activity.totalFloat)\n")
                details.append("Free Float: \(activity.freeFloat)\n")
            } else {
                details.append("ID = \(activity.id): \(activity.description)\n" )
                details.append("Early Start: \(activity.earlyStart)\n")
                details.append("Early Finish: \(activity.earlyFinish)\n")
                details.append("Late Start: \(activity.lateStart)\n")
                details.append("Late Finish: \(activity.lateFinish)\n")
                details.append("Total Float: \(activity.totalFloat)\n")
                details.append("Free Float: \(activity.freeFloat)\n")
            }
